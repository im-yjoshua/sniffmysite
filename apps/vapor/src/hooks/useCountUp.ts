...
/**
 * Count-up hook for score reveals (§2.7 — "score count-up on reveal").
...
export function useCountUp(target: number, start: boolean, duration = 1200): number {
  ...
  useEffect(() => {
    ...
    // Reduced motion: skip the choreography, show the number. The CSS
    // handles the stamp; the count-up needs the same respect here.
